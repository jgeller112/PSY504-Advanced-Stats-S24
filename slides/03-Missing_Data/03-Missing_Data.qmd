...
-   Predictive mean matching
    ...
    -   Instead of directly using this predicted mean to impute the missing value, PMM identifies a set of "donors" from the observed data. These donors are the cases whose predicted means are closest to the predicted mean of the case with the missing value
:::

::: {.column width="50%"}
![](){fig-align="center"}

![](images/PMM.PNG){width="1391"}
:::
:::
...
-   Each data point influences the estimation only for the specific parameters related to it, meaning it contributes to the part of the model where data is available.

-   The estimation process operates directly on the data that is present, without attempting to fill in or guess (impute) any missing data.
...
> A health psychologist is surveying high school students on marijuana use. Students who are users left these questions blank.
...
-   Missing data handled misproperly can bias your expectations

-   MI and ML are good ways to handle missing data!

    -   Bayesian methods are good too :)

-   Also other methods like inverse probability weighting seem to work well.
